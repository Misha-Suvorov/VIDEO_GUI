...
void ScaleHorizontal::drawScale(
    cv::InputOutputArray img, const Scalar &color, int thickness, int lineType, float value)
{
    int startX = 100;
    int startY = 60;
    int endX = 620;
    ...
    for (int i = 0; i < numTicks; i++) {
        ...
        cv::drawLineStroked(img,
                            Point(tickPosX, startY),
                            Point(tickPosX, startY - 10),
                            cv::Scalar(255, 255, 255),
                            thickness,
                            lineType);
    ...
    for (int i = 0; i < numTicks - 1; i++) {
        ...
        cv::drawLineStroked(img,
                            Point(smallTickPosX, startY),
                            Point(smallTickPosX, startY - 5),
                            cv::Scalar(255, 255, 255),
                            thickness,
                            lineType);
    }
    ...
    Point textPosition = Point(centerX - textSize.width / 2,
                               startY - 30); // Center-aligned text, static position
